...
CREATE TABLE IF NOT EXISTS pharmacies_purchase_orders (
    ...
    status VARCHAR(50) DEFAULT 'Received' CHECK (status IN ('Pending', 'Received', 'Cancelled')), -- Fulfillment status only
    created_at TIMESTAMP WITH TIME ZONE DEFAULT timezone('utc'::text, now())
);

CREATE TABLE IF NOT EXISTS pharmacy_purchase_order_items (
    id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
    purchase_order_id UUID REFERENCES pharmacies_purchase_orders(id) ON DELETE CASCADE,
